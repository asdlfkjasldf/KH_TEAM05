...
//				
//		// 총 글수
//		bCount = new MemberManagementBoardList().getBoardCount();
...
//		
//		ArrayList<MemberManagementBoard> volist = new MemberManagementBoardList().selectBoardList(startRnum,endRnum);
//		
//		
...
//		request.getRequestDispatcher("WEB-INF\\Mangement\\MemberMangementBoard.jsp").forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MemberManagementListService udetailservice = new MemberManagementListService();
		String userid = request.getParameter("userid");
		System.out.println("userid: " + userid);
		
		List<Member> membervolist  = new ArrayList<Member>(); 
		Member m = udetailservice.userDetail(userid);
		if (m != null) {
			System.out.println("유저 상세정보 조회성공");
			HttpSession session = request.getSession();
			session.setAttribute("userid", m);
		}
